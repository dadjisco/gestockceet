...
public class EntreeEnStocks extends Controller {
    ...
    public static void index() {
        List<SortieEnStock> stockList =new ArrayList<SortieEnStock>();
        List<SortieEnStock> sorties = SortieEnStock.find("beneficiaire.id=?1", Security.getAccount().id).fetch();
        List<EntreeEnStock> entrees=EntreeEnStock.find("user.account.id=?1", Security.getAccount().id).fetch();
        for(SortieEnStock s: sorties){
            for(EntreeEnStock e: entrees){
                if(s.nbCompteur>e.nbRestant){
                    stockList.add(s);
                }
            }
        }
...
//    public static void saveNewEntreeEnStock(@Required(message = "reference obligatoire") long refLivraison, 
//            @Required(message="la date est obligatoie") Date date, String nbCompteur, String qteTotaleSelect) {
//        if (validation.hasErrors()) {
//            params.flash();
//            validation.keep();
//            newEntreeEnStock(stockSelect);
//        }
//        if(Integer.parseInt(nbCompteur) > Integer.parseInt(qteTotaleSelect)){
//            validation.addError("errors", "Dépassement de quantité non autorisé!", new String[]{"errors"});
//            params.flash();
//            validation.keep();
//            newEntreeEnStock(stockSelect);
//        }else{
//                int quantite = stockUpdate(stockSelect.get(0), date, Integer.parseInt(nbCompteur), Integer.parseInt(qteTotaleSelect));
////                while (quantite>0) {            
////                for(int x=1; x<stockSelect.size(); x++) {
////                    stockUpdate(stockSelect.get(x), date, Integer.parseInt(nbCompteur), Integer.parseInt(qteTotaleSelect));
////                }
////            }   
...
//    }  
    ...
    public static void saveEntreeEnStock(
        ...
        //(1)-(2)-> LISTE DES COMPTEURS A DECLARER DANS CONTENTIEUX
            boolean rep = listSend.removeAll(compteurs);
            if(rep==true){
                int ecart = sortieEnStock.getCompteurs().size() - Integer.parseInt(nbreConfirmer);
                models.Contentieux contentieux= new models.Contentieux(ecart, new Date(), entreeEnStock);
                contentieux.setCompteurs(listSend);
                contentieux.save();
            }            
        }
        try {
